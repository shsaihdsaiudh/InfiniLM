...
> 状态:W1–W5 完成(精度/显存/性能达标,vLLM parity),W6 KV cache FP8(E4M3 动态量化)完成(2026-09-02,KV 显存 1.94× 压缩,精度三项在 ±0.5pt 噪声带内),W7 上游卫生:ascend/bang/metax/moore 四个 vendor 的 paged 算子签名透传已补齐(2026-09-03,F8 显式 NOT_IMPLEMENTED)。精度:PPL +0.082%、C-Eval +0.37pt、MMLU −0.21pt(全量),与 vLLM 0.28 同权重 parity;性能:decode bs=1 **142.96 tok/s = BF16 的 155%**(naive 的 5.02×,vLLM FP8 的 91%),bs=4 持平反超,bs≥8 落后 vLLM 3-5.4×(M_TILE tensor-core 化空间);KV FP8 decode 残留 1.05-1.27×(无 split-kv,已由 W7 抹平);显存 55.6% 达标。marlin 路线因 InfiniCore kernel sm_120 缺陷搁置(dev_fp8/marlin_sm120_issue.md)。详见 dev_fp8/w3~w6 报告。**W8 bs≥8 GEMM tensor-core 化(mma.m16n8k16 + FP8 位摆放 decode)已完成 GPU 实测(2026-09-04):算子对拍 21/21,decode bs=16 1794.6 tok/s = naive 4.20× / SIMT 4.73×,bs=32 2702.0 = naive 3.30×;cuBLASLt block-scale 路线因 12.8 工具链无对应 API 否决,自研 mma 为终点。W7 KV FP8 decode split-kv 已完成 GPU 验证(2026-09-05):算子对拍 28/28(含 alibi+split),bs=1 in=4k/16k FP8 KV decode 从 1.14×/1.27× 反超 BF16 至 0.77×/0.58×,SPLITKV=0 回归与 W6 逐点吻合,E2E 贪婪对拍逐 token 一致;KV FP8 线(W6+W7)闭环**(dev_fp8/w7_kv_fp8_splitkv_report.md / w8_mma_report.md)。后续:prefill 融合、metax 后端
