...
output="$repo_root/build/linux/x86_64/release/dsv4_moe_native_smoke"
infinilm_lib=$(find "$repo_root/build/linux/x86_64/release" \
    -maxdepth 1 -name '_infinilm.cpython-*.so' -print -quit)
if [[ -z "$infinilm_lib" ]]; then
    echo "Build _infinilm before running the MoE smoke test" >&2
    exit 1
fi
...
g++ \
    ...
    "$repo_root/dev_dsv4/moe_native_smoke.cpp" \
    "$infinilm_lib" \
    -L"$infini_root/lib" \
    -Wl,-rpath,"$infini_root/lib" \
    -Wl,-rpath,"$(dirname "$infinilm_lib")" \
    -L"$torch_lib" \
    -Wl,-rpath,"$torch_lib" \
    -linfinicore_cpp_api \
    -linfiniop \
